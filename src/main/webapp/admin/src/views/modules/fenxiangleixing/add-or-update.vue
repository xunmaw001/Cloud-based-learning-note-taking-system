<template>
	<div class="addEdit-block" :style='{"minHeight":"100vh","padding":"0px 3%","background":"url(http://codegen.caihongy.cn/20230417/f15cd3c95ab74db6bb97dd6d6b40180e.png) no-repeat center bottom / 100% 100%"}' style="width: 100%;">
		...
		>
			<template >
				<el-form-item :style='{"width":"100%","margin":"0 0 20px","background":"none","display":"inline-block"}' class="input" v-if="type!='info'"  label="分享类型" prop="fenxiangleixing">
					<el-input v-model="ruleForm.fenxiangleixing" placeholder="分享类型" clearable  :readonly="ro.fenxiangleixing"></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 20px","background":"none","display":"inline-block"}' v-else class="input" label="分享类型" prop="fenxiangleixing">
					<el-input v-model="ruleForm.fenxiangleixing" placeholder="分享类型" readonly></el-input>
				</el-form-item>
			</template>
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `fenxiangleixing/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.fenxiangleixingCrossAddOrUpdateFlag = false;
						 this.parent.search();
