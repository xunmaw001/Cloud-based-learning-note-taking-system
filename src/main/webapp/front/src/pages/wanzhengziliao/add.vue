...
<div :style='{"padding":"20px 20px 40px","margin":"10px auto 0","borderColor":"#52b646","borderRadius":"12px","background":"#fff","borderWidth":"3px","width":"1200px","position":"relative","borderStyle":"solid dashed double double"}'>
    ...
    >
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="完整资料" prop="wanzhengziliao">
            <file-upload
            tip="点击上传完整资料"
            action="file/upload"
            :limit="1"
            :multiple="true"
            :fileUrls="ruleForm.wanzhengziliao?ruleForm.wanzhengziliao:''"
            @change="wanzhengziliaoUploadChange"
            ></file-upload>
          </el-form-item>  
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="发送时间" prop="fasongshijian" >
              <el-date-picker
...
<script>
  export default {
    ...
    created() {
      ...
      this.ruleForm.fasongshijian = this.getCurDateTime()
    },
    methods: {
      getMakeZero(s) {
          return s < 10 ? '0' + s : s;
      ...
      init(type) {
        ...
        if(type=='cross'){
          ...
          for (var o in obj){
            ...
            if(o=='wanzhengziliao'){
              this.ruleForm.wanzhengziliao = obj[o];
              this.ro.wanzhengziliao = true;
              continue;
            }
            if(o=='fasongshijian'){
    ...
    // 多级联动参数
      // 多级联动参数
      info(id) {
        this.$http.get('wanzhengziliao/detail/${id}', {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
        ...
        this.$refs["ruleForm"].validate(valid => {
          if(valid) {
            ...
            if(crossrefid && crossuserid) {
                 ...
                 this.$http.get('wanzhengziliao/list', {
                  params: params
                 }).then(res => {
                     if(res.data.data.total>=crossoptnum) {
                         ...
                         // 跨表计算
                          ...
                          this.$http.post('wanzhengziliao/add', this.ruleForm).then(res => {
                              if (res.data.code == 0) {
                                  this.$message({
                                      message: '操作成功',
                                      type: 'success',
      ...
      wanzhengziliaoUploadChange(fileUrls) {
          this.ruleForm.wanzhengziliao = fileUrls.replace(new RegExp(this.$config.baseUrl,"g"),"");;
      },
    }
  };
</script>
